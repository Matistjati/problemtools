                ...
                if directory == 'accepted' and has_testcases:
                    ...
                    if fixed_limit is not None and lower_bound_runtime is not None:
                        ...
                        if lower_bound_runtime * ac_to_time_limit > fixed_limit:
                            ...
                            # We just warn when the fixed time limit comes from command line
                            if context.fixed_timelim is not None:
                                diag.warning(msg)
                            else:
                                diag.error(msg)  # ... but if it came from problem.yaml, it's an error if bounds aren't kept

                        if not math.isclose(fixed_limit, tl_from_subs) and results_table is None:
                            print(
                    ...
                    print(
                        f'   Slowest AC runtime: {_fmt_number(lower_bound_runtime)}, setting timelim to '
                        f'{_fmt_number(timelim)} secs, safety margin to {_fmt_number(timelim_margin)} secs'
                    )
                    set_timelim(timelim)
                    if results_table is not None:
