...
def test_parse_empty_2023_fails():
    with pytest.raises(ValidationError):
        metadata.parse_metadata(FormatVersion.V_2025_09, {}, {'en': 'Hello World!'})
...
def test_parse_minimal_2023(minimal_2023_conf):
    m = metadata.parse_metadata(FormatVersion.V_2025_09, minimal_2023_conf, {'en': 'Hello World!'})
    assert m.name['en'] == 'Hello World!'
    assert not m.source
    assert not m.credits.authors
...
def test_parse_typo_fails(minimal_2023_conf):
    ...
    c['limits'] = {'typo': 1}
    with pytest.raises(ValidationError):
        metadata.parse_metadata(FormatVersion.V_2025_09, c, {'en': 'Hello World!'})
...
def test_parse_single_author_2023(minimal_2023_conf):
    ...
    m = metadata.parse_metadata(FormatVersion.V_2025_09, c, {'en': 'Hello World!'})
